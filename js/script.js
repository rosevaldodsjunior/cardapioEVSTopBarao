...
// Função auxiliar segura de formatação de moeda BRL
...
const MENU_DATA = [
  ...
  // 2. BEBIDAS FUNCIONAIS
  {
    ...
    price: 65.00,
    desc: 'Nosso combo RESET de 5 dias ativa seu metabolismo, ajuda a desinchar e reduzir medidas, além de foco, disposição e energia.',
  ...
  },
  {
    ...
    price: 30.00,
    desc: 'A melhor bebida energética funcional! Proporciona mais foco, concentração, auxilia na perda de peso e elimina retenção líquida.',
  ...
  },
  {
    ...
    price: 30.00,
    desc: 'Rico em fibras ativas! Excelente para combater a retenção de líquidos e melhorar o funcionamento do intestino.',
  ...
  },
  {
    ...
    subtitle: 'Linha Especial Proteica',
    price: 23.00,
    ...
    sectionEl.innerHTML = `
      ...
      <div class="products-grid">
        ${items.map(item => {
          ...
          return `
            <div class="item-card">
              <div class="item-media" onclick="openImageZoom('${item.image}', '${item.defaultImg}', '${escapeHTML(item.name)}')">
                <img class="item-img" src="${item.image}" alt="${escapeHTML(item.name)}" onerror="this.onerror=null; this.src='${item.defaultImg}';">
                <span class="item-highlight-tag">${escapeHTML(item.subtitle)}</span>
              ...
              <div class="item-content">
                ...
                <div class="item-footer">
                  <div class="item-price-box">
                    ...
                    <span class="price-value">${formatBRL(item.price)}</span>
                  </div>
...
function openCustomizeModal(itemId) {
  ...
  document.getElementById('custom-item-base-price').innerText = `Preço base: ${formatBRL(item.price)}`;
...
function calculateCustomPrice() {
  ...
  document.getElementById('custom-total-btn-price').innerText = formatBRL(total);
}
...
function confirmCustomizedItem() {
  ...
  if (document.getElementById('chk-protein').checked) {
    selectedExtras.push('Dose Extra Proteína (+R$ 8,00)');
    extraCost += 8.00;
  }

  const border = document.querySelector('input[name="custom_border"]:checked');
  if (border && border.value !== 'Nenhuma') {
    selectedExtras.push(`${border.value} (+R$ 6,00)`);
    extraCost += 6.00;
  }

  const fiber = document.querySelector('input[name="custom_fiber"]:checked');
  if (fiber && fiber.value !== 'Nenhuma') {
    selectedExtras.push(`${fiber.value} (+R$ 9,00)`);
    extraCost += 9.00;
  }

  if (document.getElementById('chk-crunch').checked) {
    selectedExtras.push('Crunch Protéico (+R$ 6,00)');
    extraCost += 6.00;
  }
  if (document.getElementById('chk-peanut').checked) {
    selectedExtras.push('Amendoim (+R$ 3,00)');
    extraCost += 3.00;
  }
...
function updateCartCounters() {
  ...
  totalBadge.innerText = formatBRL(totalPrice);

  if (totalQty > 0) {
    floatBar.classList.remove('hidden');
    // Aciona feedback tátil de pulso
    floatBar.classList.remove('bump');
    void floatBar.offsetWidth;
    floatBar.classList.add('bump');
  } else {
...
function openCheckoutModal() {
  ...
  for (const [key, item] of Object.entries(cart)) {
    ...
    const extrasText = item.extras && item.extras.length > 0 
      ? `<div style="font-size: 0.72rem; color: var(--pdf-magenta); margin-top:2px;">+ ${escapeHTML(item.extras.join(', '))}</div>` 
      : '';
    const obsText = item.itemObs 
      ? `<div style="font-size: 0.72rem; color: var(--text-muted); font-style: italic;">Obs: ${escapeHTML(item.itemObs)}</div>` 
      : '';
    ...
    row.innerHTML = `
      <div class="row-details">
        ...
        <p>${item.qty} un. x ${formatBRL(item.finalPrice)}</p>
        ${extrasText}
        ${obsText}
      </div>
      <div class="row-pricing">
        ...
        <span class="row-total-val">${formatBRL(itemSubtotal)}</span>
      </div>
    `;
  ...
  document.getElementById('summary-subtotal-price').innerText = formatBRL(totalPrice);
  document.getElementById('summary-grand-price').innerText = formatBRL(totalPrice);
...
function dispatchOrderWhatsApp() {
  ...
  for (const [, item] of cartEntries) {
    ...
    message += `▪️ *${item.qty}x ${item.name}*\n   Unit: ${formatBRL(item.finalPrice)} | Subtotal: ${formatBRL(itemTotal)}\n`;
  ...
  message += `💰 *VALOR TOTAL: ${formatBRL(grandTotal)}*\n`;
